...
| Phase   | 이름                           | 상태 | 진행률 |
| ------- | ------------------------------ | ---- | ------ |
| Phase 4 | 어드민 보안 강화 및 역할 표시  | 대기 | 0/4    |
| Phase 5 | 사용자 경험 개선 (멤버/프로필) | 대기 | 0/8    |
| Phase 6 | 어드민 고도화 및 그룹 이미지   | 대기 | 0/9    |

### 체크리스트 요약

#### Phase 4 -- 어드민 보안 강화 및 역할 표시

- [ ] A-1: proxy.ts 어드민 접근 보안 강화 (try/catch)
- [ ] A-2: 어드민 사용자 목록에 역할(is_admin) 컬럼 추가
- [ ] A-1 검증: 비어드민 계정으로 /admin 접근 차단 확인
- [ ] A-2 검증: 역할 뱃지 정상 표시 확인

#### Phase 5 -- 사용자 경험 개선

- [ ] W-1: 그룹 상세 페이지에 멤버 목록 섹션 추가
- [ ] W-3-1: 회원가입 폼에 닉네임 필드 추가
- [ ] W-3-2: 프로필 수정 페이지 및 Server Action 구현
- [ ] W-3-3: 앱 헤더에 프로필 페이지 링크 연동
- [ ] W-1 검증: 일반 멤버 계정으로 멤버 목록 확인
- [ ] W-3 검증: 닉네임 변경 및 비밀번호 변경 확인

#### Phase 6 -- 어드민 고도화 및 그룹 이미지

- [ ] A-3-1: 어드민 사용자 상세 페이지 (조회)
- [ ] A-3-2: 어드민 사용자 닉네임 수정 기능
- [ ] A-3-3: 어드민 비밀번호 재설정 기능
- [ ] A-3-4: 어드민 계정 삭제 기능
- [ ] W-2-1: DB 컬럼 추가 + 타입 수정 (groups.image_url)
- [ ] W-2-2: Supabase Storage 버킷 설정 + 이미지 업로드 구현
- [ ] W-2-3: 그룹 생성/수정 폼에 이미지 업로드 UI 추가
- [ ] W-2-4: GroupCard, GroupHeader에 이미지 표시
- [ ] Phase 6 통합 검증: Playwright MCP E2E 테스트
...
- [ ] `createAdminClient()` 호출부를 try/catch로 감싸기
- [ ] catch 블록에서 `/admin/login`으로 리다이렉트 처리
- [ ] is_admin 체크 실패 시에도 안전하게 리다이렉트
...
- [ ] 일반 사용자 로그인 상태에서 `/admin` 접근 -> `/admin/login` 리다이렉트 확인
- [ ] 미인증 상태에서 `/admin` 접근 -> 리다이렉트 확인
...
- [ ] `getAllUsers()` 함수 반환값에 `is_admin` 필드 포함
- [ ] 사용자 목록 테이블에 "역할" 컬럼 헤더 추가
- [ ] 역할 셀에 조건부 뱃지 렌더링 (어드민: 파란색, 일반: 회색)
...
- [ ] 그룹 상세 페이지에 멤버 목록 섹션 추가
- [ ] `getMembersByGroupId()` 호출하여 멤버 데이터 조회 (이미 구현됨)
- [ ] `MemberList` / `MemberItem` 컴포넌트 재사용 (추방 버튼은 권한 체크)
- [ ] 멤버 목록 UI: 아바타, 닉네임, 역할 뱃지 표시
...
- [ ] 일반 멤버로 그룹 상세 페이지 접근 -> 멤버 목록 섹션 존재 확인
- [ ] owner/admin 계정 -> 추방 버튼 표시 확인
- [ ] 일반 멤버 계정 -> 추방 버튼 미표시 확인
...
- [ ] 회원가입 폼에 닉네임 input 필드 추가 (선택, placeholder: "닉네임 (선택)")
- [ ] 가입 완료 후 `profiles.username` 업데이트 로직 추가
- [ ] 닉네임 유효성 검증 (최소 2자, 최대 20자, 선택 입력)
...
- [ ] `lib/actions/profile-actions.ts` 생성: `updateProfile(username)` Server Action
- [ ] `components/profile/profile-form.tsx` 생성: 닉네임 수정 폼 + 비밀번호 변경 폼
- [ ] `app/protected/profile/page.tsx` 생성: 현재 프로필 정보 표시 + 수정 폼
- [ ] 닉네임 변경: `profiles` 테이블 UPDATE (Server Action)
- [ ] 비밀번호 변경: 클라이언트 Supabase `supabase.auth.updateUser({ password })` 사용
- [ ] 현재 닉네임/이메일 프리필 표시
...
- [ ] `/protected/profile` 페이지 접근 -> 현재 닉네임/이메일 표시 확인
- [ ] 닉네임 변경 -> 성공 toast 메시지 확인
- [ ] 비밀번호 변경 (신규 비밀번호 불일치) -> 에러 메시지 확인
...
- [ ] 앱 헤더의 사용자 이름/아이콘에 `/protected/profile` 링크 추가
- [ ] 프로필 페이지 링크가 모바일/데스크탑 양쪽에서 접근 가능하도록 구현
...
- [ ] `getUserDetail(userId)` 함수 추가: 이메일, 닉네임, 가입일, 그룹 목록, 역할 반환
- [ ] `updateUserByAdmin(userId, data)` 함수 추가: 닉네임(profiles.username) 수정
- [ ] `resetPasswordByAdmin(userId, newPassword)` 함수 추가: `auth.admin.updateUserById()` 사용
- [ ] `deleteUserByAdmin(userId)` 함수 추가: `auth.admin.deleteUser()` 사용
- [ ] 상세 페이지 UI: 사용자 정보 카드 + 수정 폼 + 위험 영역(삭제)
- [ ] 사용자 목록에서 행 클릭 시 상세 페이지로 이동하는 링크 추가
- [ ] 삭제 전 확인 다이얼로그 표시
...
- [ ] `/admin/users/[userId]` 접근 -> 사용자 정보 표시 확인
- [ ] 닉네임 수정 -> 성공 후 목록 페이지에서 반영 확인
- [ ] 삭제 버튼 -> 확인 다이얼로그 표시 확인
- [ ] 삭제 확인 -> 목록 페이지로 리다이렉트 + 해당 사용자 제거 확인
...
- [ ] SQL 마이그레이션 문서화: `ALTER TABLE groups ADD COLUMN IF NOT EXISTS image_url TEXT;`
- [ ] `types/index.ts`의 `Group` 인터페이스에 `image_url?: string` 필드 추가
- [ ] Supabase 타입 재생성 (`supabase gen types typescript`)
...
- [ ] 클라이언트에서 `supabase.storage.from("group-images").upload()` 호출 유틸리티 작성
- [ ] `createGroup`, `updateGroup` Server Action에 `image_url` 처리 추가
- [ ] 이미지 파일 크기 제한 (최대 2MB) 및 형식 검증 (JPEG, PNG, WebP)
...
- [ ] 그룹 생성/수정 폼에 이미지 파일 선택 input 추가
- [ ] 이미지 미리보기 기능 구현
- [ ] `GroupCard` (목록): 이미지 있으면 썸네일 표시, 없으면 기본 색상 플레이스홀더
- [ ] `GroupHeader` (상세): 상단 배너 또는 아이콘으로 이미지 표시
- [ ] Next.js `Image` 컴포넌트 사용 (width/height 지정)
...
- [ ] 그룹 생성 폼에서 이미지 업로드 -> 미리보기 표시 확인
- [ ] 생성 완료 후 GroupCard에 썸네일 표시 확인
- [ ] 그룹 상세 페이지에서 GroupHeader에 이미지 표시 확인
- [ ] 이미지 없는 그룹 -> 플레이스홀더 정상 표시 확인
...
- [ ] 어드민 사용자 관리 전체 플로우: 목록 -> 상세 -> 수정 -> 목록 반영
- [ ] 그룹 이미지 전체 플로우: 생성(이미지 포함) -> 목록 표시 -> 상세 표시
- [ ] 프로필 수정 -> 그룹 멤버 목록에서 닉네임 반영 확인
- [ ] 에러 핸들링: 잘못된 이미지 형식 업로드 시 에러 메시지 확인
